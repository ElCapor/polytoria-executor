...
-- Animation loop using Rendered event
...
-- Using game loop for consistent clicking
game.Rendered:Connect(function(dt)
    if clicking then
        mouse_click(0)
        wait(1/cps)
...
```lua
-- Using Input singleton to get mouse world position or simulate movement
while true do
    local mousePos = Input:GetMouseWorldPosition()
    if mousePos then
        -- Move camera or player slightly to bypass AFK
        game["Players"].LocalPlayer.Rotation = game["Players"].LocalPlayer.Rotation + Vector3.New(0, 0.1, 0)
    end
    wait(30)
end
```
